...
def create_index_as_numeric(r):
    schema = (NumericField( name="User.ID", sortable=True), TagField("$.User.PASSPORT", as_name="User.PASSPORT"),  \
              TagField("$.User.Address.ID", as_name="User.Address.ID", sortable=True) )
...
def test_different_index_dialect_perf():
    ...
    for i in range(TOTAL_KEY_COUNT):
        d = cct_prepare.generate_single_object(random.randint(0 , TOTAL_KEY_COUNT) , random.randint(0 , TOTAL_KEY_COUNT), "aaa")
    ...
    print("TAG SEARCH DIALECT 1: " + str(mean(tag_search_dialect_1_times)))
